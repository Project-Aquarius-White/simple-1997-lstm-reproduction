"""
Adding Problem Animation

Visualizes the Adding Problem experiment (Section 5.4 of the 1997 paper).
Shows a sequence with two marked positions and the target formula.

Scene: AddingProblem
Duration: ~20 seconds

Run: cd assets/animations && manimgl 05_adding_problem.py AddingProblem -w
"""
import os
import sys
...
class AddingProblem(InteractiveScene):
    
    def construct(self):
        title = Text("Adding Problem (Experiment 5.4)", color=TEXT_WHITE, font_size=44)
        title.to_edge(UP, buff=0.4)
        self.play(Write(title), run_time=1.5)
        self.wait(0.5)
        
        subtitle = Text(
            "Task: Sum two marked values from a long sequence",
        ...
        target_value = 0.5 + (x1_val + x2_val) / 4.0
        
        formula = Tex(
            R"\text{Target} = 0.5 + \frac{X_1 + X_2}{4}",
            font_size=32
        )
        formula.next_to(VGroup(value_boxes, marker_boxes), DOWN, buff=0.8)
        
        result = Text(
            f"= 0.5 + ({x1_val} + {x2_val})/4 = {target_value:.2f}",
        ...
        takeaway = Text(
            "LSTM remembers marked values across 100+ timesteps",
